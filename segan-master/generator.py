...
class AEGenerator(object):
    ...
    def __call__(self, noisy_w, is_ref, spk=None, z_on=True, do_prelu=False):
        ...
        with tf.variable_scope('g_ae'):
            ...
            # enc ~
            # [16384x1, 8192x16, 4096x32, 2048x32, 1024x64, 512x64, 256x128, 128x128, 64x256, 32x256, 16x512, 8x1024]
            # dec ~
            # [8x2048, 16x1024, 32x512, 64x512, 8x256, 256x256, 512x128, 1024x128, 2048x64, 4096x64, 8192x32, 16384x1]
            # FIRST ENCODER
            ...
            # SECOND DECODER (reverse order) - fix this final layer 1 sec
            # Get all but last element in reverse, then add 1 at the end
