...
flags.DEFINE_integer("canvas_size", 2**15, "Canvas size (Def: 2^14).")
...
def main(_):
    ...
    for device in devices:
        if len(devices) > 1 and 'CPU' in device.name:
            # Use cpu only when we don't have gpus
            continue
    ...
    with tf.Session(config=config) as sess:
        ...
        if FLAGS.test_wav is None:
            ...
            wavfile.write(
                os.path.join(FLAGS.save_clean_path, wavname), 16000, c_wave)  # If wrong change 16e3 to 16000
