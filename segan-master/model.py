...
class SEGAN(Model):
    ...
    def __init__(self, sess, args, devices, infer=False, name='SEGAN'):
        ...
        self.g_dilated_blocks = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
        # num fmaps for AutoEncoder SEGAN (v1)
        self.g_enc_depths = [16, 16, 32, 32, 64, 64, 128, 128, 256, 256, 512, 1024]  # adding another layer
        # Define D fmaps
        self.d_num_fmaps = [16, 16, 32, 32, 64, 64, 128, 128, 256, 256, 512, 1024]  # adding another layer
            ...
            while not coord.should_stop():
                ...
                if (counter / num_devices) % config.save_freq == 0:
                    ...
                    for m in range(min(20, canvas_w.shape[0])):
                        ...
                        wavfile.write(
                            os.path.join(save_path, 'sample_{}-'
                        ...
                        if not os.path.exists(m_gtruth_path):
                            ...
                            wavfile.write(
                                os.path.join(save_path,
                                             'dif_{}.wav'.format(m)), int(16e3),
                                de_emph(sample_dif[m], self.preemph))
                        np.savetxt(
                            os.path.join(save_path, 'd_rl_losses.txt'),
                            d_rl_losses)
                        np.savetxt(
                            os.path.join(save_path, 'd_fk_losses.txt'),
                            d_fk_losses)
                        np.savetxt(
                            os.path.join(save_path, 'g_adv_losses.txt'),
                            g_adv_losses)
                        np.savetxt(
                            os.path.join(save_path, 'g_l1_losses.txt'),
                            g_l1_losses)
...
class SEAE(Model):
    ...
    def __init__(self, sess, args, devices, infer=False, name='SEAE'):
        super(SEAE, self).__init__(name)
        self.args = args
        self.sess = sess
        self.keep_prob = 1.
        if infer:
